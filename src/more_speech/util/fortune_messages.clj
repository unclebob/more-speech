(ns more-speech.util.fortune-messages
  (:require [clojure.string :as string]
            [more-speech.util.fortune-words :refer :all]))

(def fortune-messages
  [
   ...
   "Listen to the man behind the curtain, and see all his failures and talents."
   "The only way to go fast is to go well."])

(defn get-message []
  (rand-nth fortune-messages))

(defn next-fragment [fragment]
  (let [a-msg (rand-nth fortune-messages)
        size (count a-msg)
        a-msg (subs a-msg (rand-int (quot size 2)))
        frag-string (apply str (interpose " " fragment))
        pos (.indexOf ^String a-msg ^String frag-string)]
    (if (neg-int? pos)
      ""
      (first (drop 2 (string/split (subs a-msg pos) #" "))))))

(defn get-markov-message
  ([]
   (get-markov-message :normal))

  ([type]
   (loop [msg (take 2 (string/split (rand-nth fortune-messages) #" "))
          n 0]
     (if (or (> n 30000)
             (.endsWith (last msg) ".")
             (> (count msg) 100))
       (if (< (count msg) 5)
         ...
         (apply str (interpose
                      ...
                      (if (= type :insane)
                        (map change-word msg)
                        msg))))
       (let [candidate (rand-nth fortune-messages)
             fragment (take-last 2 msg)
             suffix (if (empty? fragment)
                      (take 2 (string/split candidate #" "))
                      (next-fragment fragment)
                      )]
         (if (empty? suffix)
           (recur msg (inc n))
           (recur (concat msg [suffix]) (inc n))))))))
