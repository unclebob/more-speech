...
(declare db private-key public-key pubkey-num)

(describe "Composing outgoing events"
  (setup-db-mem)
  (redefs-around [config/proof-of-work-default 0])
  (with private-key (num->bytes 64 314159))
  (with public-key (get-pub-key @private-key))
  (with pubkey-num (bytes->num @public-key))
  (before (set-mem :pubkey @pubkey-num)
          (set-mem :keys {:private-key (bytes->hex-string @private-key)
